    ...
    def __init__(
        ...
        self.transformer_decoder = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(
                d_model=embedding_dim, nhead=8, dim_feedforward=hidden_size, dropout=0.1
            ),
            num_layers=num_layers,
        )
        self.fc = nn.Linear(
            embedding_dim, embedding_dim
        )  # Adjust the output dimension if necessary

    def forward(self, embedding):
        # Example forward method implementation
        # This should be replaced with the actual logic for the Decoder's forward pass
        # Check if input dimensions are valid
        if embedding.size(1) != self.fc.in_features:
            raise ValueError("Input dimensions do not match the expected dimensions.")
        ...
        corpus_norm = torch.nn.functional.normalize(
            corpus_embeddings, p=2, dim=1
        ...
        top_k_similarities, top_k_indices = similarities.topk(
        ...
        for i in range(batch_size):
            ...
            if top_k_indices[i].numel() == 0:
                print(
                    f"Warning: No similar sentences found for batch item {i}. Returning defaults."
                )
            ...
            print(
                f"Batch {i} - Selected similarity:", selected_similarity
            )  # Debug Print
            ...
            decoded_results.append(
                (decoded_sentence, selected_index, selected_similarity)
            )
