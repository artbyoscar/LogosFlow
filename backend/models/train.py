...
def prepare_data_loader(embedding_file, batch_size, seq_length=32, val_split=0.1):
    ...
    try:
        ...
        print(
            "Please ensure that 'preprocess.py' has been run successfully to generate embeddings."
        )
        sys.exit(1)
...
def monitor_system(epoch, batch_idx, interval=10):
    ...
    if batch_idx % interval == 0:
        ...
        print(
            f"  Epoch: {epoch+1}, Batch: {batch_idx+1}, CPU Usage: {cpu_percent:.1f}%, Memory Usage: {memory_info.rss / 1024**2:.2f} MB"
        )
...
def main():
    ...
    train_loader, val_loader = prepare_data_loader(
        embeddings_file_path, batch_size, seq_length, val_split
    )

    # Model, Optimizer, Loss
    model = SimplePolicyNetwork(
        embedding_dim, hidden_size, num_layers, model_type, seq_length
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (input_embeddings, target_embeddings) in enumerate(train_loader):
            ...
            predicted_embeddings = predicted_embeddings / predicted_embeddings.norm(
                p=2, dim=-1, keepdim=True
            )
            target_embeddings = target_embeddings / target_embeddings.norm(
                p=2, dim=-1, keepdim=True
            )
        ...
        val_loss = 0.0
        with torch.no_grad():
            for batch_idx, (input_embeddings, target_embeddings) in enumerate(
                val_loader
            ):
        ...
        print(
            f"Epoch {epoch+1} completed in {epoch_end_time - epoch_start_time:.2f} seconds. Validation Loss: {val_loss:.4f}"
        )
