...
from backend.models.generate import beam_search  # Ensure correct import path
...
class MockModel(nn.Module):
    ...
    def forward(self, embeddings: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            embeddings (torch.Tensor): Input embeddings with shape [batch_size, seq_len, embedding_dim].

        Returns:
            torch.Tensor: Output embeddings filled with 0.5, shape [batch_size, 1, embedding_dim].
        """
...
class TestBeamSearch(unittest.TestCase):
    def setUp(self):
        """
        Set up the test environment by initializing the MockModel, creating sample embeddings,
        and defining the computation device.
        """
    ...
    def test_beam_search_dimensions(self):
        ...
        self.assertEqual(
            len(best_indices), 15, "best_indices should have a length of 15."
        )
        ...
        self.assertEqual(
            ...
            (1, 384),
            "best_embeddings should have shape (1, 384).",
        )
    ...
    def test_beam_search_output_values(self):
        ...
        self.assertTrue(
            torch.allclose(best_embeddings, expected_embedding),
    ...
    def test_beam_search_repetition_penalty(self):
        ...
        """
        # Reinitialize MockModel if needed (not necessary here since MockModel behavior is consistent)
        # self.model = MockModel(embedding_dim=384)
        ...
        # Check that best_indices have the correct length
        self.assertEqual(
            len(best_indices), 15, "best_indices should have a length of 15."
        )

        # Optionally, verify that indices are not repeating beyond expectations
        # Since MockModel returns fixed embeddings, beam_search might favor certain indices
    ...
    def test_beam_search_with_empty_corpus(self):
        ...
        with self.assertRaises(
            ValueError, msg="beam_search should raise ValueError when corpus is empty."
        ):
            beam_search(
                self.model,
                self.start_embeddings,
